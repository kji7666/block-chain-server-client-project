 ...
 * Usage for add transacation on chain: 
...
public class Chain {
     ...
     * @return
     */
    // private Block searchByBlockHash(String queryHash) {
    ...
    //  * @return
    //  */
    // private Block searchByTXID(String queryHash) {
    //     Block currentBlock = headBlock;
    //     while (currentBlock != null) {
     ...
     */
    public String getTransactionData(String height, String TXID){
        if(height != null && TXID != null){
            Block current = headBlock;
            int estimatedPathLong = current.getHeight() - Integer.valueOf(height);
            ...
            while(current != null){
                int len = current.getTransactions().length;
                for(int i = 0; i<len; i++){
                    if(current.getTransactions()[i].equals(TXID)){
                        return current.getTransactions()[i];
                    }
                }
                current = current.getNextBlock();
            }
            ...
            LOGGER.info("Height or TXID cannot is null");
        }
        return null;
    }
    ...
    public void showTransactionQueueSize(){
        System.out.println(transactionQueue.size());
    }

    /**
     * Show all transaction on each Block
     */
