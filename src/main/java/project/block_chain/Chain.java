...
public class Chain implements BlockChain{
    ...
    @Override
    public void restoreBlock(String filePath, int BlockchainLength) {
        //SHA256withRSA digital signature restore the backuped file   
        BlockchainBackupHandler backeupHandler = new BlockchainBackupHandler();
        
        try {
            this.headBlock = backeupHandler.restoreBlock(filePath+"0");
            //Since the genesisBlock is already added do the rest times
            for(int i=0; i<BlockchainLength - 4; i++){
        ...
        // call the backupmanager


        //Deletes the initial block (genesis block) and accepts the backup genesis block.
        

        //restoring the following block
    }
