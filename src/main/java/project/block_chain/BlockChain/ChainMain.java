...
class Chain {
    ...
    public synchronized void addTransaction(String transaction) {
        ...
        if (transactionQueue.size() >= NUMBER_OF_TRANSACTION_IN_BLOCK) {
    ...
    public synchronized void makeBlock() {
        try {
            String[] transactions = new String[NUMBER_OF_TRANSACTION_IN_BLOCK];
            while (true) {
                // Wait until the queue size reaches 4
                waitUntilQueueSizeReached(NUMBER_OF_TRANSACTION_IN_BLOCK); 
