...
public class BlockImpl implements Block, Serializable {
    ...
    public BlockImpl(BlockImpl previousBlock, String[] transactions){ 
        ...
        for(int i=0; i<count; i++){
            //Considering the uniqueness of TransactionID we plus the index and height of block 
            //when using merkletree search please be catious here
            this.transactionIDs[i] = SHA256.generateFileSha256(this.transactions[i]+String.valueOf(i)+String.valueOf(this.getHeight()));
        }
    ...
    private void setCurrentBlockHash(int leadingZeros, String[] preProcessDataForLeadingZero){
        ...
        if(timeout && TIMEOUT_TEST){
            System.out.println("[Timeout: 10 minutes] in creating current block. [Hegiht] "+String.valueOf(this.height));
        }
    }
